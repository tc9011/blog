...
<p id="div-border-left-yellow">层叠的重要度只是层叠规则的一部分，CSS2.1中的层叠规则如下：
...
<p id="div-border-left-yellow">CSS2.1还包括了`outline`属性，它的轮廓绘制在元素边框之上，不影响元素的大小和定位。</p>
...
<p id="div-border-left-yellow">用CSS3的`box-sizing`属性可以定义使用哪种盒模型。</p>
...
<p id="div-border-left-yellow">`border-image-slice`规定图像边框的向内偏移，从而可以精确地把图片划分成9块。
...
<p id="div-border-left-yellow">因为IE考虑元素的内容的尺寸，而不是元素本身的尺寸。在符合标准的浏览器中，如果元素的内容太大，它只会超出框之外。在IE上如果元素的内容太大，整个元素就会扩展。</p>
...
<p id="div-border-left-yellow">`background-position`使用像素为单位设置背景图像的位置时，是图像左上角到元素左上角的距离为指定像素。而使用百分数为单位时，是图像上距离图像左上角的指定百分数的点到父元素上距离父元素左上角为指定百分数距离的位置。</p>
